...
prompt_kctx() {
  ...
  if command -v kubectl > /dev/null 2>&1; then
    if [[ -f $BULLETTRAIN_KCTX_KCONFIG ]]; then
      context=$(cat $BULLETTRAIN_KCTX_KCONFIG|grep current-context| awk '{print $2}')
      namespace=$(cat $BULLETTRAIN_KCTX_KCONFIG|grep "cluster: $context" -A 1 | grep -v "$context" | awk '{print $2}')
      prompt_segment $BULLETTRAIN_KCTX_BG $BULLETTRAIN_KCTX_FG $BULLETTRAIN_KCTX_PREFIX" $namespace@$context"
    fi  
  fi
}
